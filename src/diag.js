...
// The panel is hidden for normal players and only shows when the URL carries
// ?debug — or forces itself visible on any uncaught error. It lives on <html>
// so the app rebuilding <body> can't remove it.
// ============================================================================
(function () {
  var lines = [];
  var shown = false;
  // Hard gate: the overlay is NEVER visible unless the URL has ?debug, even on
  // errors or slow-load timeouts. Normal players just see the game.
  var debugEnabled = false;
  try { debugEnabled = new URLSearchParams(location.search).has('debug'); } catch (e) { /* ignore */ }
  ...
  function render() {
    if (!debugEnabled) return;               // never touch the DOM for normal players
    try { panel().textContent = lines.join('\n'); } catch (e) { /* pre-DOM */ }
  }
  ...
  window.MPDIAG_SHOW = function () {
    if (!debugEnabled) return;               // no-op for normal players
    shown = true;
    try { panel().style.display = 'block'; } catch (e) { }
  };

  if (debugEnabled) window.MPDIAG_SHOW();
