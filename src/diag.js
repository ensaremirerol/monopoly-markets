...
(function () {
  ...
  // Only surface for an explicit ?debug — ordinary players never see it. It
  // still force-shows itself on any error (see handlers below), so a broken
  // join is never silent.
  try {
    if (new URLSearchParams(location.search).has('debug')) window.MPDIAG_SHOW();
  } catch (e) { /* ignore */ }
